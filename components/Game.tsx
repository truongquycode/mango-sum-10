...
const generateSolvableValues = (totalCells: number): number[] => {
  ...
  while (currentCount < totalCells) {
    ...
    const wantTriplet =
      (Math.random() < 0.35 || remaining === 3) && remaining >= 3;
    if (wantTriplet) {
...
const createInitialGrid = (): MangoCell[][] => {
  ...
  for (let r = 0; r < GRID_ROWS; r++) {
    ...
    for (let c = 0; c < GRID_COLS; c++) {
      ...
      const isConflict =
        currentValue === leftValue || currentValue === topValue;
        ...
        for (let k = valueIndex + 1; k < solvableValues.length; k++) {
          ...
          if (candidate !== leftValue && candidate !== topValue) {
            if ((leftValue === 5 || topValue === 5) && candidate === 5)
              continue;
            [solvableValues[valueIndex], solvableValues[k]] = [
              solvableValues[k],
              solvableValues[valueIndex],
            ];
...
export const Game: React.FC<GameProps> = ({
  ...
  const [opponentItemsStats, setOpponentItemsStats] = useState<
  ...
  const renderAvatar = (avatar: any) => {
    ...
    const displayValue =
      avatar && typeof avatar === "object" ? avatar.value : avatar || "👤";
    return (
  ...
  const playSynthSound = useCallback(
    (type: string, variant?: string) => {
      ...
      switch (type) {
        case "correct":
          osc.type = "sine";
          osc.frequency.setValueAtTime(800 + streak * 50, currTime);
          break;
        case "wrong":
          osc.type = "sawtooth";
  ...
  const playStickerSound = useCallback(
    (item: { type: string; value: string }) => {
      ...
      if (item.type === "image") {
        ...
        playTone(1046.5, now + 0.3, 0.6, "sine"); // C6 (Chốt hạ)
      } else {
        // Sticker Text
          ...
          setGrid((prev) =>
            prev.map((row, r) =>
              row.map((c, idx) => ({
                ...
                value:
                  !c.isRemoved && !remoteGrid[r][idx].isRemoved
                    ? remoteGrid[r][idx].value
                    : c.value,
              }))
            )
          );
  ...
  const getCellFromCoords = useCallback(
    (
      ...
      return {
        row: Math.max(
          0,
          Math.min(Math.floor((clientY - rect.top) / cellHeight), GRID_ROWS - 1)
        ),
        col: Math.max(
          0,
          Math.min(Math.floor((clientX - rect.left) / cellWidth), GRID_COLS - 1)
        ),
      };
    },
    []
  ...
  const handleMove = (clientX: number, clientY: number) => {
    ...
    if (pos) {
      if (
        pos.row !== dragState.currentPos?.row ||
        pos.col !== dragState.currentPos?.col
      ) {
  ...
  const handleEnd = () => {
    ...
    const isMagicValid =
      magicActive && selectedCells.length > 0 && selectedCells.length <= 4;
  ...
  const processMatch = (cellsToRemove: Position[]) => {
    ...
    setBonusText({
      text: `+${totalAdded}`,
      id: Date.now(),
      color: scoreMultiplier > 1 ? "text-green-400" : "text-yellow-400",
    });
  ...
  const handleUseItem = (item: GameItem) => {
    ...
    switch (item.type) {
      ...
      case "SPEED_UP":
        connection?.send({
          type: "ITEM_ATTACK",
          payload: { effect: "SPEED_UP" },
        } as MultiPlayerMessage);
        break;
      case "DEBUFF_SCORE":
        connection?.send({
          type: "ITEM_ATTACK",
          payload: { effect: "DEBUFF_SCORE" },
        } as MultiPlayerMessage);
        break;
      case "STEAL":
  ...
  return (
    <div className="h-full w-full flex flex-col bg-cyan-50 relative overflow-hidden font-sans select-none">
      ...
      {shuffleMessage && (
        <div className="absolute inset-0 z-[60] flex items-center justify-center bg-white/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white px-8 py-6 rounded-3xl shadow-2xl flex flex-col items-center animate-bounce border-4 border-cyan-300">
            ...
            <span className="text-cyan-600 font-black text-2xl">
              {shuffleMessage}
            </span>
          </div>
        </div>
      ...
      {/* SCREEN EFFECTS */}
      {isFrozen && (
        <div className="absolute inset-0 bg-blue-300/10 pointer-events-none z-40 border-[6px] border-blue-200 rounded-lg m-1 animate-pulse"></div>
      )}
      {speedMultiplier > 1 && (
        <div className="absolute inset-0 bg-red-300/10 pointer-events-none z-40 border-[6px] border-red-300 rounded-lg m-1"></div>
      )}
      {magicActive && (
        <div className="absolute inset-0 pointer-events-none z-40 border-[6px] border-purple-300/40 opacity-50 animate-pulse rounded-lg m-1"></div>
      )}
      ...
      <div className="relative z-50 pt-2 px-1 pb-1 shrink-0 w-full max-w-2xl mx-auto">
        ...
        <div className="w-full max-w-md mx-auto h-2 mb-2 bg-gray-200 rounded-full overflow-hidden shadow-inner relative border border-white/50">
          <div
            className={`h-full transition-all duration-1000 ease-linear rounded-full ${
              timeLeft < 10 ? "bg-red-400" : "bg-cyan-400"
            }`}
            style={{
              width: `${Math.min(
                (timeLeft / GAME_DURATION_SECONDS) * 100,
                100
              )}%`,
            }}
          />
        ...
        <div className="flex items-end justify-between w-full gap-0 px-0">
          ...
          <div className="flex flex-col items-center bg-white/60 backdrop-blur-md rounded-2xl p-2 border-2 border-white/60 shadow-lg w-28 sm:w-36 min-h-[120px] sm:min-h-[160px] relative transition-transform hover:scale-105 shrink-0">
            ...
            <span className="text-[10px] sm:text-xs font-bold text-cyan-700 mt-1 w-full text-center truncate px-1">
              {myName}
            </span>

            {/* SCORE & STREAK ROW */}
            <div className="flex items-center justify-center gap-1 flex-1 w-full mt-1 min-h-[24px]">
              <span className="text-xl sm:text-3xl font-black text-cyan-600 leading-none drop-shadow-sm">
                {score}
              </span>
              {streak > 0 && (
                <div className="flex flex-col items-center w-6 sm:w-8">
                  <span className="text-[8px] sm:text-[10px] font-bold text-orange-500 whitespace-nowrap">
                    🔥x{streak}
                  </span>
              ...
              {bonusText && (
                <span
                  ...
                  className={`absolute top-0 left-1/2 -translate-x-1/2 ${
                    bonusText.color || "text-yellow-400"
                  } font-black text-lg animate-float-up pointer-events-none whitespace-nowrap z-50 drop-shadow-sm`}
                >
                  {bonusText.text}
                </span>
              )}
          ...
          <div className="flex-1 flex flex-col items-center justify-end h-full pb-1 gap-1 overflow-hidden">
            ...
            <div className="w-full flex items-center justify-center gap-0 opacity-80 mb-auto mt-2">
              {/* Left Line: 0 -> 100 */}
              <svg
                className="flex-1 h-6 text-red-400"
                preserveAspectRatio="none"
                viewBox="0 0 100 20"
              >
                <path
                  d="M0,10 L70,10 L75,0 L80,20 L85,10 L100,10"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
              ...
              <div className="text-xl sm:text-2xl animate-heartbeat text-red-500 drop-shadow-sm z-10 -mx-1">
                ❤️
              </div>
              {/* Right Line: 0 -> 100 */}
              <svg
              ...
              >
                <path
                  d="M0,10 L15,10 L20,0 L25,20 L30,10 L100,10"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  className="animate-pulse"
            ...
            {isMultiplayer ? (
                    ...
                    return (
                      <div key={index} className="relative group">
                        <button
                          ...
                          className={`w-7 h-7 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-sm sm:text-lg shadow-sm transition-all active:scale-90 ${
                            item
                              ? `${
                                  ITEM_CONFIG[item.type].color
                                } text-white border-2 border-white hover:scale-110 shadow-md`
                        ...
                        {item && (
                          <svg className="absolute inset-0 w-full h-full -rotate-90 pointer-events-none p-0.5">
                            <circle
                              ...
                              strokeDasharray="100"
                              strokeDashoffset={
                                100 * ((Date.now() - item.receivedAt) / 60000)
                              }
              ...
              <div className="opacity-30 text-2xl sm:text-3xl animate-pulse pb-2">
                🍋
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col items-center bg-white/60 backdrop-blur-md rounded-2xl p-2 border-2 border-white/60 shadow-lg w-28 sm:w-36 min-h-[120px] sm:min-h-[160px] relative transition-transform hover:scale-105 shrink-0">
            ...
            <span className="text-[10px] sm:text-xs font-bold text-gray-500 mt-1 w-full text-center truncate px-1">
              {opponentName}
            </span>

            {/* SCORE & TIME ROW (Cân bằng với bên trái) */}
            <div className="flex items-center justify-center gap-2 flex-1 w-full mt-1 min-h-[24px]">
              <span className="text-xl sm:text-3xl font-black text-gray-600 drop-shadow-sm">
                {opponentScore}
              </span>
              <span
          ...
          >
            ...
            {dragState.isDragging &&
              ...
              dragState.currentPos && (
                <div
                  className={`absolute pointer-events-none border-[5px] rounded-2xl z-50 shadow-[0_0_15px_rgba(255,255,255,0.6)] ${
                    isValidSum || magicActive
                  ...
                  style={{
                    ...
                    width: `${
                      (Math.abs(
                        dragState.currentPos.col - dragState.startPos.col
                      ) +
                        1) *
                      (100 / GRID_COLS)
                    }%`,
                    height: `${
                      (Math.abs(
                        dragState.currentPos.row - dragState.startPos.row
                      ) +
                        1) *
                      (100 / GRID_ROWS)
                    }%`,
            ...
            {grid.map((row, r) =>
              row.map((cell, c) => (
                ...
                >
                  {/* Sử dụng biến FruitIcon đã tạo ở Bước 2 */}
                  <FruitIcon
                    value={cell.value}
                    isSelected={isCellSelected(r, c)}
                    isRemoved={cell.isRemoved || false} // Thêm || false cho chắc chắn
      ...
      <div className="shrink-0 h-14 flex items-center justify-center px-4 pb-2 w-full relative z-50 gap-4">
        {!isMultiplayer && (
          <button
            ...
            className="bg-white/80 hover:bg-white text-cyan-600 shadow-lg shadow-cyan-100/50 border-b-4 border-cyan-200 active:border-b-0 active:translate-y-1 px-6 py-2 rounded-2xl font-black text-sm uppercase tracking-wider backdrop-blur-sm transition-all"
          >
            Chơi Lại
          </button>
        )}
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-bold text-sm transition-all shadow-lg active:scale-95 border-b-4 active:border-b-0 active:translate-y-1 ${
            isMuted
              ? "bg-red-100 text-red-500 border-red-200"
              : "bg-white/80 text-cyan-600 border-cyan-200"
          }`}
        >
          {isMuted ? "🔇" : "🔊"}
        </button>
