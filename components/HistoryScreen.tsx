...
export const HistoryScreen: React.FC<HistoryScreenProps> = ({
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setHistory(
          parsed.sort(
            (a: MatchRecord, b: MatchRecord) => b.timestamp - a.timestamp
          )
        );
      } catch (e) {
  ...
  const getResultDisplay = (status: string) => {
    switch (status) {
      case "SOLO":
        return { text: "CHƠI ĐƠN", color: "text-gray-500", bg: "bg-gray-100" };
      case "WIN":
        return { text: "THẮNG", color: "text-green-600", bg: "bg-green-100" };
      case "LOSE":
        return { text: "THUA", color: "text-red-600", bg: "bg-red-100" };
      case "DRAW":
        return { text: "HÒA", color: "text-yellow-600", bg: "bg-yellow-100" };
      default:
        return { text: "???", color: "text-gray-500", bg: "bg-gray-100" };
    }
  };
    ...
    return (
      ...
      >
        {Object.entries(items).map(([type, count]) => (
          <div
            key={type}
            className="flex items-center gap-1 bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200"
            title={ITEM_CONFIG[type as ItemType]?.name}
          >
            <span className="text-xs">
              {ITEM_CONFIG[type as ItemType]?.icon}
            </span>
            {count > 1 && (
      ...
      <div className="z-10 w-full max-w-lg h-full flex flex-col">
        <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-xl border-4 border-cyan-200 flex flex-col h-full overflow-hidden">
          ...
          <div className="p-4 border-b-2 border-gray-100 flex items-center justify-between bg-white shrink-0">
            <h2 className="text-2xl font-black text-cyan-600 uppercase tracking-wide">
              Lịch Sử Đấu 📜
            </h2>
            <button
              onClick={onBack}
              className="w-8 h-8 rounded-full bg-gray-100 text-gray-500 hover:bg-red-100 hover:text-red-500 font-bold flex items-center justify-center transition-all active:scale-90"
            >
              ✕
            </button>
          </div>
          ...
          <div className="bg-gradient-to-r from-cyan-400 to-blue-400 p-4 text-white shrink-0 shadow-lg relative overflow-hidden">
            ...
            <div className="flex justify-between items-center text-center relative z-10">
              <div className="flex-1 px-2 border-r border-white/20">
                ...
                <div className="text-[10px] uppercase opacity-90 font-bold">
                  Trận đấu
                </div>
              </div>
              <div className="flex-1 px-2 border-r border-white/20">
              ...
              <div className="flex-1 px-2">
                <div className="text-2xl font-black text-green-100 drop-shadow-md">
                  ...
                  <span className="text-red-100">{stats.losses}</span>
                </div>
                <div className="text-[10px] uppercase opacity-90 font-bold">
                  Thắng/Thua
                </div>
          ...
          <div className="p-3 bg-gray-50 border-b border-gray-200 flex flex-wrap gap-2 items-center justify-center relative z-30">
            ...
            <div className="relative group min-w-[160px]" ref={calendarRef}>
              <button
                ...
                className={`w-full pl-10 pr-4 py-2 rounded-full border-2 bg-white shadow-sm flex items-center h-[42px] transition-all relative z-10 ${
                  showCalendar
                ...
                <div className="absolute top-full left-0 mt-2 bg-white rounded-2xl shadow-2xl border-2 border-cyan-100 p-3 w-72 z-50 animate-fade-in">
                  <div className="flex justify-between items-center mb-3">
                    <button
                      ...
                      className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center font-bold text-gray-500"
                    >
                      ←
                    </button>
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    ...
                    {Array.from({
                      ...
                      const currentM = (viewDate.getMonth() + 1)
                        .toString()
                        .padStart(2, "0");
            ...
            <div className="relative flex-1 min-w-[140px]" ref={dropdownRef}>
              <button
                ...
                className={`w-full pl-10 pr-8 py-2 rounded-full border-2 bg-white shadow-sm flex items-center h-[42px] transition-all relative z-10 text-left ${
                  showDropdown
                    ? "border-cyan-400 ring-2 ring-cyan-100"
                    : "border-cyan-100 hover:border-cyan-300"
                }`}
            ...
            {filteredHistory.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-400 animate-fade-in">
                ...
                <button
                  ...
                  className="mt-2 text-cyan-500 text-sm underline hover:text-cyan-600"
                >
                    ...
                    <div className="flex justify-between items-center mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-cyan-600 bg-cyan-50 px-2 py-1 rounded-full border border-cyan-100 flex items-center gap-1 font-bold">
                          <span>⏱</span>
                          <span>
                            {match.duration || GAME_DURATION_SECONDS}s
                          </span>
                        </span>
                      </div>
                      <span
                        className={`text-[10px] font-black px-3 py-1 rounded-full shadow-sm ${result.bg} ${result.color}`}
                      >
                        {result.text}
                      </span>
                    </div>
          ...
          <div className="p-4 bg-white border-t border-gray-100 shrink-0">
            <Button
              onClick={onBack}
              className="w-full py-3 rounded-2xl shadow-lg shadow-cyan-200/50"
              variant="secondary"
            >
              Quay Lại Menu
            </Button>
